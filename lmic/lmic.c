...
static void rxschedInit (xref2rxsched_t rxsched) {
    ...
    u1_t intvExp = rxsched->intvExp; //间隔指数,beacon窗口划分为128份,间隔的份数的指数 12 - (1~7)   ping 数量= 2^(7-intvExp)
    ostime_t off = os_rlsbf2(LMIC.frame) & (0x0FFF >> (7 - intvExp)/* 2^12/2^(1~7) */); // random offset (slot units)
...
static bit_t rxschedNext (xref2rxsched_t rxsched, ostime_t cando) {
  again:
    ...
    if( (slot=rxsched->slot) >= 128 )
        return 0;
    u1_t intv = 1<<rxsched->intvExp/* 2^(7-k) */;
    if( (rxsched->slot = (slot += (intv))) >= 128 )
        return 0;
    rxsched->rxtime = rxsched->rxbase
        + ((BCN_WINDOW_osticks * (ostime_t)slot) >> BCN_INTV_exp) /* BCN_WINDOW_osticks/ping数量 */
        - calcRxWindow(/*secs BCN_RESERVE*/2+slot+intv,rxsched->dr); /* 时钟偏差校正 */
    rxsched->rxsyms = LMIC.rxsyms;
    goto again;
}
